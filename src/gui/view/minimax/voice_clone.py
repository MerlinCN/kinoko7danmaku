...
class MinimaxVoiceCloneInterface(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.source_audio_upload_widget.fileUploaded.connect(self._on_source_file_uploaded)
        ...
        # 说明文本
        example_desc = CaptionLabel('上传一段 <8 秒的示例音频，并填写音频中念的台词，用于克隆音色')
        ...
        self.example_audio_upload_widget.fileUploaded.connect(self._on_example_file_uploaded)
        self.example_audio_upload_widget.fileRemoved.connect(self._on_example_file_removed)
        ...
        self.example_description_edit.setPlaceholderText('请输入示例音频中念的完整台词，需与音频内容一致并以标点结尾')
        ...
        self.test_text_edit.setPlaceholderText('输入测试文本，点击播放按钮预览音色效果...')
    ...
    def _on_source_file_uploaded(self, file_response: Any, duration: float) -> None:
        ...
        """
        logger.info(f'源音频文件上传成功: {file_response.file.filename} ({duration:.1f}秒)')
    ...
    async def _on_generate_clicked(self) -> None:
        ...
        # 显示处理状态
        state_tooltip = StateToolTip('正在生成', '正在生成音色，请稍候...', self.window())
    ...
    def _show_audio_preview(self, voice_id: str, voice_name: str, audio_url: str) -> None:
        """显示音频预览区域
