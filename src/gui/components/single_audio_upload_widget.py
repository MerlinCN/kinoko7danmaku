...
class SingleAudioUploadWidget(CardWidget):
    ...
    async def _upload_file(self, file_path: str) -> None:
        ...
        try:
            # 调用上传 API
            response = await self._tts_service.upload_audio_file(file_path, purpose=self._purpose)
    ...
    def _show_file_info(self, file_response: FileUploadResponse, duration: float) -> None:
        """显示文件信息
