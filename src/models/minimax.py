...
class PronunciationDict(BaseModel):
    ...
    tone: list[str] = Field(default_factory=list, description="音调列表，格式: '文字/(拼音)'")
...
class MinimaxTTSRequest(BaseModel):
    ...
    pronunciation_dict: PronunciationDict | None = Field(default=None, description='发音字典')
...
class MinimaxTTSResponse(BaseModel):
    ...
    base_resp: dict[str, Any] | None = Field(default=None, description='基础响应信息')
...
class VoiceCloneRequest(BaseModel):
    ...
    need_volume_normalization: bool = Field(default=False, description='是否启用音量归一化')
